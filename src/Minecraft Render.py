'''
This is kind of incomplete but, I want to upload what I have currently for the "minecraft" game in Python turtle. 
There are still a bunch of bugs to hammer out such as blocks improperly in some conditions and rotating camera up and down being buggy
'''
...
camera_pos = [0, 1, -5]  # camera position in 3D space, -5z starting pos and 1y
...
# game controls/game envirornment
...
def game_ui(): # initialized the game ui, such as hotbar, coords, viewmodel/hand, ect
    global sprinting
    # draw crosshair
    penup()
    seth(0)
...
def times_of_day():
    ...
    screen.ontimer(times_of_day, 60000) # function recursion cuz while loops are poop
...
def check_collision(new_position):
    ...
    for obj in objects:
        ...
        # seperate top and bottom hitbox. top larger to act as player hitbox. scuffed but works lol
...
def render_objects():
    ...
    def bubble_sort_descending(arr): # thx stack overflow
        n = len(arr)
        for i in range(n):
    ...
    # loop apply matrix math, clipping bs, and colour to each object
    for obj in sorted_objects:
        ...
        for Vertex in Vertices:
            ...
            # apply perspective projection. i didnt use the perspective_projection() function here, cuz it wasnt fucking working bruh
            if zPos != 0:
            ...
            # apply clipping. without this you will see 2 objects, if its half way clipped. schizo shit
            if zPos < near_clip or zPos > far_clip: 
                continue
            # store projected vertex
            projected_vertices.append([x_screen, y_screen, zPos])
        
        face_color = obj.get("color", "green") # later add another attribute for colour for each individual block
        faces = [
            [0, 1, 2, 3],  # front face
            [4, 5, 6, 7],  # back face
            [0, 1, 5, 4],  # bottom face
    ...
    if len(projected_vertices) == 0:
        clear()
    penup()
    game_ui() # call game ui, this will include hotbar, crosshair, health, armour, food, view model ect
    screen.update()
...
def keyInput():
    ...
    screen.onkey(lambda: rotate_camera("down", 2), "k") # looking up and down is fucked, will be fixed later
