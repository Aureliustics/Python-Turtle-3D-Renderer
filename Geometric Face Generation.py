...
def position_handler(axis, value):
...
def keyInput():
    key_actions = {
        "Up": lambda: angle_handler('X', -0.1),
        "Down": lambda: angle_handler('X', 0.1),
        "Right": lambda: angle_handler('Y', 0.1),
        "Left": lambda: angle_handler('Y', -0.1),
        "w": lambda: position_handler('Z', 10),
        "s": lambda: position_handler('Z', -5) if z_axis >= 5 else None,
        "d": lambda: position_handler('X', 5),
        "a": lambda: position_handler('X', -5),
        "Space": lambda: position_handler('Y', 5),
        "Control": lambda: position_handler('Y', -5)
    }
